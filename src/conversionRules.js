...
// Restructure the conversionRules object to include a data property
...
export default function registerResources(server) {
  ...
  server.registerResource('conversionRules', {
    ...
    data: conversionRules.data,  // Note: Using .data here to match the structure
    accessors: {
      // Define accessor methods to get specific rules
      getElementFindingRules: () => conversionRules.data.elementFinding,
      getGestureRules: () => conversionRules.data.gestures,
      getApplicationRules: () => conversionRules.data.application
    }
  });
