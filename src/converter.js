 ...
 */
async function convertCode(code, platform = 'android', rulesResource) {
    try {
        ...
        console.error(`Found ${allRuleCategories.length} rule categories:`, allRuleCategories);
        ...
        console.error(`Total rules collected: ${allRules.length}`);
 ...
 */
async function convertFile(filePath, outputPath, platform = 'android', rulesResource) {
    try {
 ...
 */
async function validateCode(code, rulesResource) {
    try {
        ...
        allRuleCategories.forEach(category => {
            if (Array.isArray(rulesResource.data[category])) {
                ...
                const hasMatch = matchingPatterns.some(pattern => categoryPatterns.includes(pattern));
                if (hasMatch) {
...
export { convertCode, convertFile, validateCode };